...
- 公式の会場地図に、お気に入りと通路をたどった巡回順を重ねた**巡回シート**（そのまま PDF）
- 1件ごとに **優先度・予算・お使いメモ**を書き込める。お使いメモを書いた分だけの**お使いリスト**も出せる
- サークル情報（頒布物・価格・書店リンク）の CSV / JSON 出力
...
![巡回シート](docs/images/sheet.jpg)

配置図は公式の地図。色が付いているのがお気に入りで、破線が巡回順、丸数字が回る順番。破線は通路をたどっているので机の上は横切らない。

地図タイルが取れなかったときは、スペースの矩形座標から起こした簡易な図に自動で落ちる。「表示を調整」の**「公式の地図を使う」**を外せば常に簡易な図になる（PDF は軽くなる）。

![配置図](docs/images/map.jpg)
...
巡回シートと CSV / JSON は API から直接取るので、下の「表示モード」や「収集」は使わなくてよい。以下はサイトの画面から拾うフォールバック（パネルの「そのほかの操作」の中）の話。

### 先に表示モードを合わせる（画面から拾う場合のみ）
...
| 操作 | 内容 |
| --- | --- |
| 画面分だけ | いま DOM 上にあるカードを保存する |
| 最後まで送って拾う | ページ末尾の読み込みトリガまで自動スクロールしながら拾う |
...
```
wcid, スペース, 地区, ホール, ブロック, 番号, 枝番, サークル名, 執筆者, ジャンル,
お気に入り, 色番号, サイトのメモ, 優先度, 予算, お使いメモ,
頒布物の価格合計, 頒布物, 新刊, タグ, 書店・外部, 配置表記,
サークル更新日時, カット画像URL, 詳細URL, 取得日時
```

JSON は頒布物や外部リンクを入れ子のまま出す。
...
- または一覧ページ右下のパネル →「巡回シートを開く」

拡張機能のページとして開くので、書き込んだ内容が保存され、リロードしても消えない。

**中身**

- **公式の会場地図そのもの**。サイトが配信している地図タイルを貼り合わせて敷いている。ブロック記号・スペース番号・ジャンル帯・ホール表記（東1/E1 など）・入口・トイレまで公式のまま
...
**スペースとサークル名を押すと、そのサークルの詳細ページが別タブで開く。** 旧版（classic）にサークル1件の実ページ `https://classic-webcatalog.circle.ms/Circle/<wcid>` があるので、そこへの普通のリンク。
...
パネルの **CSV / JSON** を押すと、その場でお気に入りを取り直してから書き出す。取込ボタンを別に押す必要はない。ログインが切れている等で取れないときは、手元にある分で出す。
...
3. 同じブロックのお気に入りを島にまとめ、1件ずつ「そのスペースの前に立てる通路のマス」を求める
4. 島どうしの距離を**通路上のダイクストラ**で測る（斜め移動あり、机の角のすり抜けは禁止）
5. その距離行列で最近傍法 → 2-opt
6. 島の中は、前の島から入ってきた側に近い端から順に並べる（蛇行）

**地図に描く線は、立ち位置から立ち位置へ通路をたどった実際の道筋。** 島と島のあいだも、島の中も、区間はすべて通路探索でつないでいる。同じブロックでも列をまたいで離れていることがあるため、島の中を直線で結ぶと机を突っ切ってしまう。番号は経路が実際に来る場所（島の入口）に置いてある。

入口の位置は API から取れないので、南側の端を起点とみなしている。

ヘッダには歩く距離と、歩くのにかかる時間の目安を出す。距離は「スペース1つ＝約 0.9 m」、時間は「人混みを時速 3km」として換算したもので、並ぶ時間は含まない。

1件ずつ全部の前を通る前提で測っているので、それなりの数字になる。実データでは 2日目 東123ホールの 227 サークルで約 3,070 m・歩くだけで約 61 分だった。
...
| GET | `/api/v2/MapTiles?path=tile256/minimap/…` | 配置図のジオメトリ（JSON） |
| GET | `/MapTiles/tile256/40/{kind}Day{n}-{area}/{x}-{y}.png?t=…` | 会場地図のタイル（PNG）。`kind` は `""`（下地）と `overlay-`（文字や記号）。両方重ねて1枚にする |
...
- **サークル1件のページは旧版にある。** `https://classic-webcatalog.circle.ms/Circle/<wcid>`
  が普通のページとして開ける（title もサークル名になる）。新しい方のサイトは SPA で、
  詳細は一覧のカードを押すと右から出るドロワー。開いても URL が変わらないうえ、
  読み込み時に自分でクエリを書き直すのでハッシュに目印を載せる手も効かない。
  リンクとして貼るなら旧版のほうを使う
- `?wcid=` は一覧を絞る条件ではなく「そこへスクロールする」指定でしかないので、
  1ページ目(100件)に入っていないサークルには届かない
- 頒布物の `price` は `priceDisplay` が true のときだけ表に出す。値段自体は入っていても
  サークルが伏せている場合がある
- **イベント ID を `window.__NUXT__` から読んではいけない。** Nuxt はハイドレーションが
  済むとこれを消すので、コンテンツスクリプトが起きた頃には無い。API のリクエスト URL には
  必ず `event_id=` が乗っているので、通信を覗いてそこから拾う
- ページ本体からの受け口は、カードが出るのを待たずにページを開いた直後に張る。
  イベント ID を載せた通信は一覧が描画される前に終わっている
- 地図タイルは 256px、`BigmapWidth`/`BigmapHeight` を割った枚数ぶん並ぶ（東123 で 13×7）。
  座標系は配置ジオメトリと同じなので、そのまま重ねられる。
  **未ログインだと画像ではなく SPA の HTML が返る**（200 で 1.7KB）ので、
  拡張のページからは `credentials` を付けて取り、Content-Type を見て弾く。
  URL の組み立てには `print-` という種類も出てくるが、実際に叩くと 404 だった
...
| 経路が全部の島に届くか | 各島の番号が経路上にあることを確認（7 エリアすべてで取り残しなし）。通行可能マスが 1 つに繋がっていること、お気に入り 381 件すべての立ち位置がそこに含まれることも確認 |
| 公式の地図 | 7 エリアすべてでタイルが貼れることを確認。既知のスペースの矩形と地図上の位置が一致することも目視で確認 |
| イベントの切り替わり | 保存値をわざと別の値にしてからサイトを開き、実イベントの ID に戻ることを確認。次のコミケが始まっても設定を触らずに済む |
| 価格 | 381 件中 111 件に公開価格があり、一括で予算に入れて ¥294,000。リロード後も保持 |
| サークルの詳細ページ | 3 件で `classic-webcatalog.circle.ms/Circle/<wcid>` が 200 を返し、title がそのサークル名になることを確認 |
...
src/
  ...
  lib/maptiles.js        公式の地図タイルを貼り合わせて配置図の下敷きにする
