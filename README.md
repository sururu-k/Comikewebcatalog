...
**「配置図つき巡回シートを作る」**を押すと、サイトの API からお気に入り・サークル情報・配置図のジオメトリを取り直し、次を含む1枚のページを新しいタブで開く。

- **実際の会場配置図**（スペースの矩形座標から起こした SVG。画像タイルではないので拡大しても潰れず、PDF にしても軽い）
- お気に入りをお気に入り色で塗り、周囲に色枠を重ねて見つけやすくしたもの
- 巡回順を結んだ経路線と、島ごとの順番バッジ
- 島ごとのチェックリスト（スペース / サークル名 / 執筆者 / メモ / 色）

ページ右上の「PDF で保存 / 印刷」を押すとブラウザの印刷ダイアログが出るので、送信先を「PDF に保存」にすれば PDF になる。A4 横で組んである。
...
| PDF | 生成したシートを A4 横で PDF 化（約 600KB） |
...
src/
  ...
  content/favorites.js   API からお気に入りを組み立てて計画とシートに渡す
  content/sheet.js       配置図の SVG 生成と印刷用シート
