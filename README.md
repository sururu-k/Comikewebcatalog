...
入口の位置は API から取れないので、南側の端を起点とみなしている。ヘッダに歩く距離の目安と、ブロック名順に回った場合と比べて何 % 短いかを出す。実データでは大きいホールで 16〜25% 短くなった。

距離は「スペース1つ＝約 0.9 m」として換算した目安。通路を通る前提で測っているので、直線で測った値よりかなり長く出る（2日目 東123ホールで直線 496m に対し通路 752m）。
...
| 巡回順 | ブロック名順と比べて 東123ホール 25% / 西12ホール 23% / 東7ホール 16% 短縮 |
